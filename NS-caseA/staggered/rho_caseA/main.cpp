...
void Fluid::print_u(){
    for (unsigned int i = 1; i < Nx-1; i++){
        double x = start_x + i * dx;
        std::cout << std::setprecision(14) << x << " " << q[i] / ((rho[i] + rho[i-1]) / 2.0) << std::endl;
    }
    std::cout << std::endl;
}
...
void Fluid::integrate_by_Euler(){
    ...
    std::vector<double> rho_new; rho_new.resize(Nx-1);
    // navier-stokes equation.
    // advection term.
    for (unsigned int i = 1; i < Nx-1; i++){
        if (q[i] >= 0.0){
            // average density.
            double rho_i = (rho[i-1] + rho[i]) / 2.0;
            q_new[i] = q[i] - dt * ((q[i] * q[i] / rho_i) - (q[i-1] * q[i-1] / rho_i)) / dx;
        } else {
            // average density.
            double rho_i = (rho[i-1] + rho[i]) / 2.0;
            q_new[i] = q[i] - dt * ((q[i+1] * q[i+1] / rho_i) - (q[i] * q[i] / rho_i)) / dx;
        }
    }
    // pressure gradient term and viscous term.
    for (unsigned int i = 2; i < Nx-2; i++){
        double u_e = q[i-1] / ((rho[i-2] + rho[i-1]) / 2.0);
        double u_c = q[i] / ((rho[i-1] + rho[i]) / 2.0);
        double u_w = q[i+1] / ((rho[i] + rho[i+1]) / 2.0);
        q_new[i] += dt * (- R * T * (rho[i] - rho[i-1]) / dx + mu * (u_e - 2.0 * u_c + u_w) / (dx * dx));
    }
    ...
    if(rho_const){
        ...
        for(unsigned int i = 1; i < Nx-2; i++){
            rho_new[i] = rho[i] - dt * (q[i+1] - q[i]) / dx;
        }
    }
    // rho boundary condition. Neumann 0.
    rho_new[0] = rho[1];
    rho_new[Nx-2] = rho[Nx-3];
    // update.
    q = q_new;
    rho = rho_new;
    time += dt;
