...
void Fluid::integrate_by_Euler(){
    ...
    for (unsigned int i = 2; i < Nx-2; i++){
        if (u[i] >= 0.0){
            double phi_e = (3.0 * u[i+1] + 6.0 * u[i] - u[i-1]) / 8.0;
            double phi_w = (3.0 * u[i] + 6.0 * u[i-1] - u[i-2]) / 8.0;
            u_new[i] += - dt * u[i] * (phi_e - phi_w) / dx;
        } else {
            double phi_e = (3.0 * u[i] + 6.0 * u[i+1] - u[i+2]) / 8.0;
            double phi_w = (3.0 * u[i-1] + 6.0 * u[i] - u[i+1]) / 8.0;
            u_new[i] += - dt * u[i] * (phi_e - phi_w) / dx;
        }
    }
